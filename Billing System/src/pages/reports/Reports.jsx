...
export default function Reports() {
  ...
  useEffect(() => {
    setLoading(true);
    const loaders = {
      sales: () => reportsApi.sales(salesPeriod).then(setSalesData),
      stock: () => reportsApi.stock().then(setStockData),
      purchase: () => reportsApi.purchase().then(setPurchaseData),
      grn: () => reportsApi.grn().then(setGrnData),
      debtors: () => reportsApi.debtors().then(setDebtorsData),
    };
    loaders[activeTab]()
  ...
  }, [debtorsData, debtorSearch, debtorFilter]);

  const outstandingDebtors = debtorsData.filter((d) => d.creditStatus === 'Outstanding');
  ...
  const lowStockItems = stockData.filter((p) => p.status === 'Low Stock' || p.status === 'Out of Stock');

  const handleExport = async (type) => {
    const map = { Sales: 'sales', Stock: 'stock', Purchase: 'purchase', GRN: 'grn', 'Credit Debtors': 'debtors' };
    try {
      await reportsApi.exportCsv(map[type] || type.toLowerCase(), salesPeriod);
  ...
  const salesKey = salesPeriod === 'daily' ? 'label' : 'label';
  ...
  return (
    <div>
      ...
      {loading && <p className="mb-4 text-sm text-slate-400">Loading report…</p>}
      ...
      {activeTab === 'sales' && (
        <div className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button type="button" onClick={() => handleExport('Sales')} className="btn-secondary !text-xs"><Download className="h-3.5 w-3.5" /> Export CSV</button>
          </div>
      ...
      {activeTab === 'stock' && (
        <div className="space-y-6">
          <div className="flex justify-end">
            <button type="button" onClick={() => handleExport('Stock')} className="btn-secondary !text-xs"><FileSpreadsheet className="h-3.5 w-3.5" /> Export CSV</button>
          </div>
          <div className="glass-card p-6">
            <h3 className="mb-4 text-sm font-semibold">Current Inventory Summary</h3>
            <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-800/50"><p className="text-xs text-slate-500">Total Products</p><p className="text-2xl font-bold">{stockData.length}</p></div>
              <div className="rounded-xl bg-emerald-50 p-4 dark:bg-emerald-950/30"><p className="text-xs text-emerald-600">In Stock</p><p className="text-2xl font-bold text-emerald-700">{stockData.filter((p) => p.status === 'In Stock').length}</p></div>
              <div className="rounded-xl bg-amber-50 p-4 dark:bg-amber-950/30"><p className="text-xs text-amber-600">Low Stock</p><p className="text-2xl font-bold text-amber-700">{stockData.filter((p) => p.status === 'Low Stock').length}</p></div>
              <div className="rounded-xl bg-red-50 p-4 dark:bg-red-950/30"><p className="text-xs text-red-600">Out of Stock</p><p className="text-2xl font-bold text-red-700">{stockData.filter((p) => p.status === 'Out of Stock').length}</p></div>
            </div>
            <h3 className="mb-4 text-sm font-semibold">Low / Out of Stock Items</h3>
            <DataTable
              columns={[
                { key: 'code', label: 'Code' },
                { key: 'name', label: 'Product' },
                { key: 'quantity', label: 'Units', render: (row) => <span className="font-bold text-red-500">{row.quantity}</span> },
      ...
      {activeTab === 'purchase' && (
        <div className="space-y-6">
          <div className="flex justify-end">
            <button type="button" onClick={() => handleExport('Purchase')} className="btn-secondary !text-xs"><Download className="h-3.5 w-3.5" /> Export CSV</button>
          </div>
      ...
      {activeTab === 'grn' && (
        <div className="space-y-6">
          <div className="flex justify-end">
            <button type="button" onClick={() => handleExport('GRN')} className="btn-secondary !text-xs"><Download className="h-3.5 w-3.5" /> Export CSV</button>
          </div>
          <div className="glass-card p-4">
            <DataTable
              columns={[
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="glass-card p-5">
              ...
              <p className="mt-2 text-2xl font-bold">{debtorsData.length}</p>
            </div>
            <div className="glass-card p-5">
          ...
          <div className="glass-card p-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
              <SearchBar value={debtorSearch} onChange={setDebtorSearch} placeholder="Search debtor, invoice, or mobile..." className="flex-1" />
              ...
              <button type="button" onClick={() => handleExport('Credit Debtors')} className="btn-secondary !text-xs shrink-0">
          ...
          <div className="glass-card p-4">
            ...
            <DataTable
              columns={[
                ...
                { key: 'creditStatus', label: 'Status', render: (row) => <StatusBadge status={row.creditStatus === 'Paid' ? 'Completed' : 'Pending'} /> },
              ]}
